...
export function computeDiff(left: string, right: string): DiffLine[] {
  ...
  while (leftIndex < leftLines.length || rightIndex < rightLines.length) {
    if (leftIndex >= leftLines.length) {
      ...
      let leftMatch = -1;
      for (let i = leftIndex + 1; i < Math.min(leftIndex + 20, leftLines.length); i++) {
      ...
      if (bestMatch !== -1 && bestMatch === rightIndex + 1 && leftMatch === -1) {
        ...
        rightIndex++;
      } else if (leftMatch !== -1 && leftMatch === leftIndex + 1 && bestMatch === -1) {
...
function computeCharDiff(left: string, right: string): { left: CharDiff[]; right: CharDiff[] } {
  ...
  const result: { left: CharDiff[]; right: CharDiff[] } = { left: [], right: [] };
  ...
  while (leftIdx < leftTokens.length || rightIdx < rightTokens.length) {
    if (leftIdx >= leftTokens.length) {
      ...
      if (result.right.length > 0 && result.right[result.right.length - 1].type === 'insert') {
        result.right[result.right.length - 1].text += rightTokens[rightIdx];
      } else {
        result.right.push({ type: 'insert', text: rightTokens[rightIdx] });
      }
      rightIdx++;
    } else if (rightIdx >= rightTokens.length) {
      // Only left side has tokens
      if (result.left.length > 0 && result.left[result.left.length - 1].type === 'delete') {
      ...
      // Tokens match
      if (result.left.length > 0 && result.left[result.left.length - 1].type === 'equal') {
        result.left[result.left.length - 1].text += leftTokens[leftIdx];
      } else {
        result.left.push({ type: 'equal', text: leftTokens[leftIdx] });
      }
      if (result.right.length > 0 && result.right[result.right.length - 1].type === 'equal') {
        result.right[result.right.length - 1].text += rightTokens[rightIdx];
      } else {
        result.right.push({ type: 'equal', text: rightTokens[rightIdx] });
      }
      leftIdx++;
      rightIdx++;
    } else {
      ...
      const maxLookAhead = Math.min(10, Math.max(leftTokens.length - leftIdx, rightTokens.length - rightIdx));
      
      for (let lookAhead = 1; lookAhead <= maxLookAhead && !foundMatch; lookAhead++) {
        if (leftIdx + lookAhead < leftTokens.length && leftTokens[leftIdx + lookAhead] === rightTokens[rightIdx]) {
          ...
          leftIdx++;
          foundMatch = true;
        } else if (rightIdx + lookAhead < rightTokens.length && leftTokens[leftIdx] === rightTokens[rightIdx + lookAhead]) {
